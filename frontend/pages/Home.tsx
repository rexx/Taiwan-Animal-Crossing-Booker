...
import { api, cleanThreadsUrl } from '../lib/api.ts';
...
export const Home: React.FC<HomeProps> = ({ onSent, notify }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.postReply(cleanedUrl);
      if (res.error === 'ALREADY_REPLIED') {
        notify('此貼文已有回覆', 'error');
        onSent(res.post_id);
      } else if (res.error === 'RATE_LIMITED') {
        notify(`系統忙碌，請 ${res.retry_after} 秒後再試`, 'error');
      } else if (res.success) {
        notify('成功送出！');
        onSent(res.post_id);
      } else {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-12 py-10">
      <div className="space-y-6">
        <p className="text-ink/60 text-xl max-w-lg mx-auto font-medium">貼上目標貼文 URL，我們將前往留下證書。</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-lg space-y-6">
        <div className="relative group">
          <input
            ...
            placeholder="https://threads.com/@user/post/..."
