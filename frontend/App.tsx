...
export const App: React.FC = () => {
  const [currentRoute, setCurrentRoute] = useState<'list' | 'detail' | 'admin'>('list');
  ...
  const navigate = (route: 'list' | 'detail' | 'admin', id?: string) => {
  ...
  return (
    <div className="min-h-screen bg-cream text-ink font-sans">
      {notification && (
        <div className={`fixed top-4 left-1/2 -translate-x-1/2 z-[100] px-6 py-3 rounded-full shadow-xl border transition-all animate-bounce ${notification.type === 'success' ? 'bg-olive text-white border-olive/20' : 'bg-red-700 text-white border-red-600'
          }`}>
          {notification.msg}
        </div>
      )}
      ...
      <nav className="border-b border-tan sticky top-0 bg-paper/80 backdrop-blur-md z-50">
        <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
          <button onClick={() => navigate('list')} className="flex items-center gap-2 group">
